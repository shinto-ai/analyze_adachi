...
def analyze_data(data_frames: List[pd.DataFrame]) -> float:
    """(従来の処理) データフレームのリストを分析し、カイ二乗値を返す"""
    try:
        total_df = process_data_frames(data_frames)
        cross_tab = create_cross_tabulation(total_df)
        expected_freq = calculate_expected_frequencies(cross_tab)
...
def analyze_all_data(all_sheets_list: List[List[pd.DataFrame]], all_single_row: bool) -> Tuple[float, Optional[pd.DataFrame], Optional[pd.DataFrame]]:
    """
    all_single_rowがTrueなら全ファイル分をまとめて1回のカイ二乗値計算し、
    その際にクロス集計表と期待値も戻り値として返す。
    Falseなら従来通りファイルごとにカイ二乗値計算後合算し、クロス集計表と期待値はNoneを返す。
    
    Returns:
        (chi_square, cross_tab, expected_freq)
    """
    if all_single_row:
        # 全ファイルについて、シートを合算して1行のDFを作り、それらを縦方向に結合
        file_sums = []
        for sheets in all_sheets_list:
            file_df = process_data_frames(sheets)  # 1行のDF
            file_sums.append(file_df)
        combined_df = pd.concat(file_sums, axis=0)
        
        # ここで1回のみカイ二乗計算
        cross_tab = create_cross_tabulation(combined_df)
        expected_freq = calculate_expected_frequencies(cross_tab)
        chi_square, _ = calculate_chi_square(cross_tab, expected_freq)
        return chi_square, cross_tab, expected_freq
    else:
        # 従来どおり各ファイルのカイ二乗値合計
        chi_square_sum = sum(analyze_data(sheets) for sheets in all_sheets_list)
        return chi_square_sum, None, None
...
def main():
    ...
    try:
        ...
        for file in excel_files:
            ...
            # 行数判定
            for df in sheets:
                if df.shape[0] > 1:  # 行数2行以上のシートがあれば
                    all_single_row = False
        ...
        # 2行以上の場合はファイルごとにクロス集計・期待度数を保存（従来処理）
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)
            
        if not all_single_row:
            print("\n各ファイルについてクロス集計表・期待度数を計算しています...")
            for i, sheets in enumerate(all_sheets_list):
                total_df = process_data_frames(sheets)
                cross_tab = create_cross_tabulation(total_df)
                expected_freq = calculate_expected_frequencies(cross_tab)
                ...
                # クロス集計表の保存
                cross_tab_filename = f"{base_filename}_crosstab_{timestamp}.xlsx"
                cross_tab_path = os.path.join(output_dir, cross_tab_filename)
        ...
        # 元のカイ二乗値の計算（all_single_rowに応じて処理分岐）
        print("\n元のデータのカイ二乗値を計算しています...")
        original_chi_square, original_crosstab, original_expected = analyze_all_data(all_sheets_list, all_single_row)
        print(f"元のデータの全体カイ二乗値: {original_chi_square:.6f}")
        
        # 1行のみの場合は、ここでcombined cross_tabとexpectedを出力
        if all_single_row and original_crosstab is not None and original_expected is not None:
            timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
            combined_crosstab_filename = f"crosstab_{timestamp}.xlsx"
            combined_expected_filename = f"expected_{timestamp}.xlsx"
            combined_crosstab_path = os.path.join(output_dir, combined_crosstab_filename)
            combined_expected_path = os.path.join(output_dir, combined_expected_filename)
            
            original_crosstab.to_excel(combined_crosstab_path)
            original_expected.to_excel(combined_expected_path)
            print("1行シナリオ用の合計クロス集計表・期待度数をoutputフォルダに保存しました。")
        ...
        try:
            for i in range(n_iterations):
                ...
                # ランダム化したデータの分析
                random_chi_square = 0.0
                if all_single_row:
                    # 全て1行の場合は、ファイルごとの合計を縦に結合して1回のカイ二乗計算
                    file_sums = []
                    for group in random_groups:
                        df_sum = process_data_frames(group)
                        file_sums.append(df_sum)
                    combined_df = pd.concat(file_sums, axis=0)
                    ctab = create_cross_tabulation(combined_df)
                    expected_freq = calculate_expected_frequencies(ctab)
                    random_chi_square, _ = calculate_chi_square(ctab, expected_freq)
                else:
                    # 従来方式
                    random_chi_square = sum(analyze_data(group) for group in random_groups)
