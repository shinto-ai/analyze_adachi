...
さらに、指定された試行回数分、各ファイルのシートをランダムに振り分けた場合のカイ二乗値を求め、これらを元のカイ二乗値と比較することで統計的有意性を評価します。
...
- demo_row1 : 行数が1の場合の動作確認。行数が1の場合と行数が2以上の場合は、カイ二乗値の求め方が異なる。
...
1. ターミナルまたはコマンドプロンプトを開き、プログラムが配置されているフォルダに移動します。
   ...
   ※ VSCodeから該当フォルダを開いて画面左上からターミナルを起動した場合、自動で移動しているので1の操作は必要ありません。
...
なお、各シートのデータが**1行の場合**と**2行以上の場合**でカイ二乗値の求め方が異なりますので注意してください。

1. プログラムを起動すると、`data`フォルダ内のExcelファイルが読み込まれます。
2. 各Excelファイルの有効なシートデータが取得されます。(全てのセルが空欄のシートなどはスキップされます。)

※ 以下3~6は、各シートのデータが**1行の場合**の処理です。(基本的なカイ二乗値の求め方)

3. 各Excelファイルごとに全シートを合計します。
4. 各Excelファイルの合計を行方向に並べて一つのクロス集計表を作成し、`output`フォルダに保存します。(ファイル名 : crosstab)
5. クロス集計表から各セルの期待度数を求め、`output`フォルダに保存します。(ファイル名 : expected)
6. クロス集計表と期待度数から、実験データ全体のカイ二乗値を求めます。

※ 以下3~6は、各シートのデータが**2行以上の場合**の処理です。

3. 各Excelファイルごとにクロス集計表の作成を行い、`output`フォルダに保存します。(ファイル名 : crosstab)
4. クロス集計表から各セルの期待度数を求め、`output`フォルダに保存します。(ファイル名 : expected)
5. 元のデータとその期待度数から、**まずは行ごとに**カイ二乗値を求め、各行のカイ二乗値を全て合計することでそのExcelファイルのカイ二乗値とします。
6. さらに、各Excelファイルごとのカイ二乗値を合計することで、実験データ全体のカイ二乗値とします。

※ 以降は、1行の場合、2行以上の場合のどちらにおいても共通の処理です。
